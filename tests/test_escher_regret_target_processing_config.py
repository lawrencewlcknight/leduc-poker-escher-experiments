"""Configuration checks for the ESCHER regret-target processing ablation."""

from __future__ import annotations

from experiments.leduc_poker.escher_regret_target_processing_ablation.config import (
    BASELINE_VARIANT_ID,
    DEFAULT_CONFIG,
    DEFAULT_SEEDS,
    VARIANTS,
...
def test_regret_target_processing_baseline_matches_carry_forward_config():
    ...
    assert DEFAULT_CONFIG["regret_target_processing"] == "none"
    assert DEFAULT_CONFIG["baseline_variant_id"] == BASELINE_VARIANT_ID


def test_regret_target_processing_variants_cover_deep_cfr_analogue():
    modes = {
        variant["variant_id"]: variant["regret_target_processing"]
        for variant in VARIANTS
    }

    assert modes == {
        "raw_regret_targets": "none",
        "standardized_regret_targets": "standardize",
        "clipped_regret_targets": "clip",
        "standardized_clipped_regret_targets": "standardize_clip",
    }


def test_regret_target_processing_variants_are_configurable():
    for variant in VARIANTS:
        config = make_variant_config(DEFAULT_CONFIG, variant)

        assert config["variant_id"] == variant["variant_id"]
        assert config["regret_target_processing"] == variant["regret_target_processing"]
