"""Configuration checks for Experiment 33 regret action-head capacity ablation."""

from __future__ import annotations

from experiments.leduc_poker.escher_regret_action_head_capacity_ablation.config import (
    BASELINE_VARIANT_ID,
    DEFAULT_CONFIG,
    DEFAULT_SEEDS,
    VARIANTS,
...
def test_regret_action_head_capacity_uses_experiment_28_candidate_baseline():
...
def test_regret_action_head_capacity_variants_match_requested_schedule():
    settings = {
        variant["variant_id"]: (
            variant["policy_network_head_depth"],
            variant["policy_network_head_units"],
            variant["regret_network_head_depth"],
            variant["regret_network_head_units"],
        )
        for variant in VARIANTS
    }

    assert settings == {
        "baseline_regret_head_64": (0, None, 1, 64),
        "regret_head_128": (0, None, 1, 128),
        "regret_head_64_64": (0, None, 2, 64),
    }


def test_regret_action_head_capacity_variants_change_only_regret_head_capacity():
