#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 48."""
...
MODULE = "experiments.leduc_poker.escher_all_large_memory_distillation_audit.cloud"
TASK_COUNT = 3
...
def build_job(args) -> dict:
    ...
    replacements = {
        "gcp/run_escher_frozen_policy_distillation_audit.sh": (
            "gcp/run_escher_all_large_memory_distillation_audit.sh"
        ),
        "EXP45_REMOTE_CONTROLLER": "EXP48_REMOTE_CONTROLLER",
        "exp45-controller": "exp48-controller",
        "Experiment 45": "Experiment 48",
    }
    for source, target in replacements.items():
        script = script.replace(source, target)
    runnable["script"]["text"] = script
    job["labels"] = {
        "experiment": "escher-all-large-memory-audit",
        "stage": args.kind,
    }
    return job
