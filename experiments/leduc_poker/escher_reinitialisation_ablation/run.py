"""CLI for Experiment 29 ESCHER reinitialisation ablation."""
...
_LOGGER = logging.getLogger("escher_poker.experiment.reinitialisation_ablation")
...
def _selected_variants(args) -> List[Dict]:
    ...
    if unknown:
        raise ValueError(f"Unknown reinitialisation variant id(s): {unknown}")
...
def _augment_summary(summary: Dict, config: Mapping) -> Dict:
    ...
    summary["reinitialisation_mode"] = (
        "reinitialised"
        if config.get("reinitialize_regret_networks") and config.get("reinitialize_value_network")
        else "persistent_regret_value"
    )
...
def _plot_final_exploitability(
    ...
    set_chart_title(ax, "ESCHER reinitialisation ablation: final exploitability")
    ax.legend()
    fig.tight_layout()
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Leduc poker ESCHER reinitialisation ablation."
    )
    parser.add_argument("--output-root", default="outputs/reinitialisation_ablation")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    with tqdm(total=total, desc="Reinitialisation variants") as progress:
        for seed in seeds:
            for variant in variants:
