"""CLI entry point for the ESCHER disk-backed regret-memory ablation."""
...
PAIRED_METRICS = [
    "delta_final_exploitability_disk_minus_baseline",
    "delta_best_exploitability_disk_minus_baseline",
    "delta_final_window_mean_exploitability_disk_minus_baseline",
    "delta_final_policy_value_disk_minus_baseline",
    "delta_final_policy_value_error_disk_minus_baseline",
    "delta_exploitability_auc_by_iteration_disk_minus_baseline",
    "delta_exploitability_auc_by_nodes_disk_minus_baseline",
    "delta_final_wall_clock_seconds_disk_minus_baseline",
    "delta_peak_rss_mb_disk_minus_baseline",
    "delta_regret_storage_mb_disk_minus_baseline",
]
...
def _storage_mb(bytes_value: float) -> float:
    return float(bytes_value) / (1024.0 ** 2)


def _make_storage_paths(run_dir: Path, config: Dict[str, Any], seed: int) -> Dict[str, Any]:
    config = dict(config)
    replay_dir = run_dir / "replay" / config["variant_id"] / f"seed_{seed}"
    if config.get("use_disk_average_policy_memory", True):
        average_dir = replay_dir / "average_policy_memories"
        average_dir.mkdir(parents=True, exist_ok=True)
        config["save_average_policy_memories"] = str(average_dir)
    if config.get("use_disk_regret_memory", False):
        config["save_regret_memories"] = str(replay_dir / "regret_memories")
    return config


def run_single_seed_variant(seed: int, config: Dict[str, Any], run_dir: Path) -> Dict[str, Any]:
    """Run one seed for one regret-memory storage variant."""
    config = _make_storage_paths(run_dir, config, seed)
    set_seed_tf(seed)
    ...
    storage_bytes = diagnostics.get(
        "regret_storage_bytes_total",
        np.asarray([0.0] * len(iterations), dtype=float),
    ).astype(float)
    ...
    summary = {
        ...
        "seed": int(seed),
        "status": "completed",
        "use_disk_regret_memory": bool(config.get("use_disk_regret_memory", False)),
        "use_disk_average_policy_memory": bool(
        ...
        "final_regret_storage_bytes": _safe_last(storage_bytes),
        "final_regret_storage_mb": _storage_mb(_safe_last(storage_bytes)),
    ...
    for idx, iteration in enumerate(iterations):
        row = {
            ...
            "regret_storage_mb": (
                _storage_mb(storage_bytes[idx]) if idx < len(storage_bytes) else np.nan
            ),
        }
...
def _paired_differences(summary_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for seed in sorted(baseline_seeds & treatment_seeds):
        ...
        rows.append({
            ...
            "seed": int(seed),
            "delta_final_exploitability_disk_minus_baseline": float(
                treatment["final_exploitability"] - baseline["final_exploitability"]
            ),
            "delta_best_exploitability_disk_minus_baseline": float(
                treatment["best_exploitability"] - baseline["best_exploitability"]
            ),
            "delta_final_window_mean_exploitability_disk_minus_baseline": float(
                treatment["final_window_mean_exploitability"]
                - baseline["final_window_mean_exploitability"]
            ),
            "delta_final_policy_value_disk_minus_baseline": float(
                treatment["final_policy_value"] - baseline["final_policy_value"]
            ),
            "delta_final_policy_value_error_disk_minus_baseline": float(
                treatment["final_policy_value_error"]
                - baseline["final_policy_value_error"]
            ),
            "delta_exploitability_auc_by_iteration_disk_minus_baseline": float(
                treatment["exploitability_auc_by_iteration"]
                - baseline["exploitability_auc_by_iteration"]
            ),
            "delta_exploitability_auc_by_nodes_disk_minus_baseline": float(
                treatment["exploitability_auc_by_nodes"]
                - baseline["exploitability_auc_by_nodes"]
            ),
            "delta_final_wall_clock_seconds_disk_minus_baseline": float(
                treatment["final_wall_clock_seconds"]
                - baseline["final_wall_clock_seconds"]
            ),
            "delta_peak_rss_mb_disk_minus_baseline": float(
                treatment["peak_rss_mb"] - baseline["peak_rss_mb"]
            ),
            "delta_regret_storage_mb_disk_minus_baseline": float(
                treatment["final_regret_storage_mb"]
                - baseline["final_regret_storage_mb"]
            ),
        })
    return rows
...
def _plot_paired_delta(paired_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    ax.scatter(
        ...
        [row["delta_final_exploitability_disk_minus_baseline"] for row in paired_rows],
        s=50,
    )
    ax.set_xlabel("Seed")
    ax.set_ylabel("Disk-backed - baseline final exploitability")
    set_chart_title(ax, "Paired effect of disk-backed regret replay")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(
        run_dir / "paired_final_exploitability_delta_disk_minus_baseline.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def _plot_outputs(
    ...
    _plot_curve(
        ...
        "ESCHER regret-memory storage ablation: exploitability",
    ...
    _plot_curve(
        ...
        "exploitability",
        "Exploitability (NashConv/2)",
        "ESCHER regret-memory storage ablation: exploitability by nodes",
        "exploitability_by_nodes_regret_memory_ablation.png",
        run_dir,
        x_col="nodes_touched",
    )
    _plot_curve(
        ...
        "ESCHER regret-memory storage ablation: average policy value",
    ...
    _plot_curve(
        ...
        "average_policy_value",
        "Average policy value",
        "ESCHER regret-memory storage ablation: average policy value by nodes",
        "average_policy_value_by_nodes_regret_memory_ablation.png",
        run_dir,
        x_col="nodes_touched",
    )
    _plot_curve(
        ...
        "ESCHER regret-memory storage ablation: policy-value error",
        "policy_value_error_regret_memory_ablation.png",
        run_dir,
    )
    _plot_curve(
        curve_rows,
        "regret_storage_mb",
        "Regret replay disk storage (MB)",
        "ESCHER regret-memory storage ablation: disk footprint",
        "regret_disk_storage_over_time.png",
        run_dir,
    )
    for key, title, filename in [
        ("policy_loss", "Average-policy network loss", "policy_loss_regret_memory_ablation.png"),
        ("value_loss", "History-value train loss", "value_loss_regret_memory_ablation.png"),
        ("value_test_loss", "History-value test loss", "value_test_loss_regret_memory_ablation.png"),
        ("regret_loss_player_0", "Regret-network loss player 0", "regret_loss_player_0_regret_memory_ablation.png"),
        ("regret_loss_player_1", "Regret-network loss player 1", "regret_loss_player_1_regret_memory_ablation.png"),
    ]:
    ...
    _plot_bar(
        ...
        "ESCHER regret-memory storage ablation: final exploitability",
        "final_exploitability_regret_memory_ablation.png",
        run_dir,
    )
    _plot_bar(
        summary_rows,
        ...
        "ESCHER regret-memory storage ablation: final average policy value",
    ...
    _plot_bar(
        ...
        "ESCHER regret-memory storage ablation: peak RSS",
    ...
    _plot_bar(
        summary_rows,
        "final_regret_storage_mb",
        "Final regret replay disk storage (MB)",
        "ESCHER regret-memory storage ablation: disk footprint",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Leduc poker ESCHER disk-backed regret-memory ablation."
    )
    ...
    parser.add_argument("--tfrecord-compression", default=None)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    _write_csv(run_dir / "seed_summary.csv", summary_rows, [
        "variant_id", "seed", "status", "use_disk_regret_memory",
        "use_disk_average_policy_memory", "final_exploitability",
        "final_policy_value", "final_policy_value_error", "final_regret_storage_mb", "peak_rss_mb",
    ])
    ...
    _write_csv(run_dir / "paired_differences_vs_baseline.csv", paired_rows, [
        "variant_id", "seed", "delta_final_exploitability_disk_minus_baseline",
        "delta_final_window_mean_exploitability_disk_minus_baseline",
        "delta_final_policy_value_disk_minus_baseline",
        "delta_final_policy_value_error_disk_minus_baseline",
        "delta_final_wall_clock_seconds_disk_minus_baseline",
    ...
    _write_csv(run_dir / "checkpoint_curves.csv", curve_rows, [
        ...
        "regret_storage_bytes_total", "regret_storage_mb", "peak_rss_mb",
    ])

    with open(run_dir / "aggregate_summary.json", "w", encoding="utf-8") as f:
        json.dump(json_safe({
            "by_variant": aggregate_rows,
            "paired_disk_minus_baseline": paired_summary_rows,
        }), f, indent=2)
    ...
    with open(run_dir / "experiment_metadata.json", "w", encoding="utf-8") as f:
        json.dump(json_safe({
            ...
            "implementation_note": (
                "Average-policy replay is disk-backed in both arms; the intended "
                "treatment is the regret-memory storage backend."
            ),
    ...
    if results:
        np.savez_compressed(
            run_dir / "regret_memory_ablation_curves.npz",
