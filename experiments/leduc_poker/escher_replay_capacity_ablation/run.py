...
_LOGGER = logging.getLogger("escher_poker.experiment.replay_capacity_ablation")
...
def _selected_variants(args) -> List[Dict]:
    ...
    if unknown:
        raise ValueError(f"Unknown replay-capacity variant id(s): {unknown}")
...
def _augment_summary(summary: Dict, config: Mapping) -> Dict:
    ...
    summary["replay_capacity_variant"] = int(config.get("memory_capacity", 0))
    summary["regret_batch_variant"] = int(config.get("batch_size_regret", 0))
    summary["value_batch_variant"] = int(config.get("batch_size_value", 0))
...
def _plot_final_exploitability(
    ...
    set_chart_title(ax, "ESCHER replay-capacity ablation: final exploitability")
    ax.legend()
    fig.tight_layout()
...
def _plot_curves(
    ...
    plot_specs = [
        (
            ...
            "ESCHER replay-capacity ablation: exploitability by nodes touched",
        ...
        (
            ...
            "ESCHER replay-capacity ablation: average-policy value by nodes touched",
        ...
        (
            ...
            "ESCHER replay-capacity ablation: policy-value error by nodes touched",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-root", default="outputs/replay_capacity_ablation")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    with tqdm(total=total, desc="Replay-capacity variants") as progress:
        for seed in seeds:
            for variant in variants:
