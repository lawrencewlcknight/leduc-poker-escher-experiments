"""CLI entry point for the ESCHER reach-weighting ablation."""
...
METRICS_TO_SUMMARISE = [
    ...
    "avg_policy_reach_mean",
    "avg_policy_reach_std",
    "avg_policy_reach_min",
    "avg_policy_reach_max",
]

PAIRED_METRICS = [
    "delta_final_exploitability_reach_minus_baseline",
    "delta_best_exploitability_reach_minus_baseline",
    "delta_final_window_mean_exploitability_reach_minus_baseline",
    "delta_final_policy_value_reach_minus_baseline",
    "delta_final_policy_value_error_reach_minus_baseline",
    "delta_exploitability_auc_by_iteration_reach_minus_baseline",
    "delta_exploitability_auc_by_nodes_reach_minus_baseline",
    "delta_wall_clock_seconds_reach_minus_baseline",
]
...
def _reach_stats(solver) -> Dict[str, float]:
    values = []
    for serialized in solver.get_average_policy_memories():
        try:
            parsed = tf.io.parse_single_example(
                serialized,
                solver._average_policy_feature_description,
            )
            values.append(float(parsed["reach_prob"].numpy()[0]))
        except Exception:
            continue
    arr = np.asarray(values, dtype=np.float64)
    return {
        "avg_policy_reach_mean": float(np.mean(arr)) if arr.size else np.nan,
        "avg_policy_reach_std": (
            float(np.std(arr, ddof=1)) if arr.size > 1 else np.nan
        ),
        "avg_policy_reach_min": float(np.min(arr)) if arr.size else np.nan,
        "avg_policy_reach_max": float(np.max(arr)) if arr.size else np.nan,
    }
...
def run_single_seed_variant(
    ...
    """Run one seed for one reach-weighting variant."""
    set_seed_tf(seed)
    ...
    reach_stats = _reach_stats(solver)
    summary = {
        ...
        "use_reach_weighted_avg_policy_loss": bool(
            config["use_reach_weighted_avg_policy_loss"]
        ),
...
def _plot_final_bar(summary_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(run_dir / "final_exploitability_reach_ablation.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def _plot_final_policy_value_bar(summary_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(run_dir / "final_average_policy_value_reach_ablation.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def _plot_paired_delta(paired_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    ax.scatter(
        ...
        [row["delta_final_exploitability_reach_minus_baseline"] for row in paired_rows],
        s=50,
    )
    ax.set_xlabel("Seed")
    ...
    fig.tight_layout()
    fig.savefig(
        run_dir / "paired_final_exploitability_delta_reach_minus_baseline.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def _plot_outputs(
    ...
    _plot_curve(
        ...
        "ESCHER reach-weighting ablation: exploitability",
    ...
    _plot_curve(
        ...
        "exploitability",
        "Exploitability (NashConv/2)",
        "ESCHER reach-weighting ablation: exploitability by nodes",
        "exploitability_by_nodes_reach_ablation.png",
        run_dir,
        x_col="nodes_touched",
    )
    _plot_curve(
        ...
        "ESCHER reach-weighting ablation: average policy value",
    ...
    _plot_curve(
        ...
        "average_policy_value",
        "Average policy value",
        "ESCHER reach-weighting ablation: average policy value by nodes",
        "average_policy_value_by_nodes_reach_ablation.png",
        run_dir,
        x_col="nodes_touched",
    )
    _plot_curve(
        ...
        "ESCHER reach-weighting ablation: policy-value error",
        "policy_value_error_reach_ablation.png",
        run_dir,
    )
    for key, title, filename in [
        ("policy_loss", "Average-policy network loss", "policy_loss_reach_ablation.png"),
        ("value_loss", "History-value train loss", "value_loss_reach_ablation.png"),
        ("value_test_loss", "History-value test loss", "value_test_loss_reach_ablation.png"),
    ]:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Leduc poker ESCHER reach-weighting ablation."
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    _write_csv(run_dir / "paired_differences_vs_baseline.csv", paired_rows, [
        "variant_id", "seed", "delta_final_exploitability_reach_minus_baseline",
        "delta_best_exploitability_reach_minus_baseline",
        "delta_final_window_mean_exploitability_reach_minus_baseline",
        "delta_final_policy_value_reach_minus_baseline",
        "delta_final_policy_value_error_reach_minus_baseline",
    ...
    with open(run_dir / "aggregate_summary.json", "w", encoding="utf-8") as f:
        json.dump(json_safe({
            ...
            "paired_reach_minus_baseline": paired_summary_rows,
        }), f, indent=2)
    ...
    with open(run_dir / "experiment_metadata.json", "w", encoding="utf-8") as f:
        json.dump(json_safe({
            ...
            "implementation_note": (
                "Reach probabilities are acting-player reach probabilities excluding "
                "chance; reach multipliers are mean-normalised in each policy batch."
            ),
    ...
    if results:
        np.savez_compressed(
            run_dir / "reach_weighting_ablation_curves.npz",
