...
def _plot_final_bar(summary_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    set_chart_title(ax, "ESCHER value-trajectory reuse ablation: final exploitability")
...
def _plot_final_policy_value_bar(summary_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    set_chart_title(ax, "ESCHER value-trajectory reuse ablation: final average policy value")
    ax.grid(True, axis="y", alpha=0.3)
    ax.tick_params(axis="x", rotation=20)
    ax.legend()
    fig.tight_layout()
...
def _plot_paired_delta(paired_rows: List[Dict[str, Any]], run_dir: Path) -> None:
    ...
    set_chart_title(ax, "Paired effect of reusing value trajectories")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Leduc poker ESCHER value-trajectory reuse ablation."
    )
