"""CLI entry point for single-seed ESCHER network-size sweeps."""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a single-seed ESCHER network-size sweep."
    )
    parser.add_argument("--output-root", default="outputs/network_size_sweeps")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    parser.add_argument(
        "--variant-ids",
        default=None,
        help="Comma-separated variant IDs. Defaults to all network-size variants.",
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if unknown:
        raise ValueError(f"Unknown network-size variant id(s): {unknown}")
    ...
    results = []
    failures = []
    for variant in tqdm(selected_variants, desc="Network-size variants"):
    ...
    if not results:
        _LOGGER.error("No network-size variants completed successfully.")
        return 1
    ...
    _LOGGER.info("Saved network-size sweep outputs to: %s", run_dir.resolve())
