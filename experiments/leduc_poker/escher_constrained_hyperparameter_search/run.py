...
def _run_hyperparameter_seed_subprocess(
    ...
    command = [
        sys.executable,
        "-m",
        "experiments.leduc_poker.escher_constrained_hyperparameter_search.run",
        "--worker-input-json",
        str(worker_input),
        "--worker-output-json",
        str(worker_output),
...
def _plot_stage_curves(results: List[Dict[str, Any]], stage_name: str, run_dir: Path) -> None:
    ...
    set_chart_title(ax, f"ESCHER Hyperparameter Search: {stage_name.title()} Exploitability")
    ax.grid(True)
    ...
    ax.set_ylabel("Exploitability (NashConv/2)")
    set_chart_title(ax, f"ESCHER Hyperparameter Search: {stage_name.title()} Exploitability by Nodes")
    ax.grid(True)
    ax.legend(fontsize=8)
    fig.tight_layout()
    ...
    set_chart_title(ax, f"ESCHER Hyperparameter Search: {stage_name.title()} Average Policy Value")
    ax.grid(True)
    ax.legend(fontsize=8)
    fig.tight_layout()
    ...
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Average policy value")
    set_chart_title(ax, f"ESCHER Hyperparameter Search: {stage_name.title()} Average Policy Value by Nodes")
    ax.grid(True)
    ax.legend(fontsize=8)
    fig.tight_layout()
...
def _plot_final_metric_bars(
    ...
    set_chart_title(ax, f"ESCHER Hyperparameter Search: {stage_name.title()} {ylabel}")
    ax.grid(True, axis="y")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Leduc poker ESCHER constrained hyperparameter search."
    )
