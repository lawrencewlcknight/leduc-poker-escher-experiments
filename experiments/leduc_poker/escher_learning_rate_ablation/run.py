...
_LOGGER = logging.getLogger("escher_poker.experiment.learning_rate_ablation")
...
def _selected_variants(args) -> List[Dict]:
    ...
    if unknown:
        raise ValueError(f"Unknown learning-rate variant id(s): {unknown}")
...
def _augment_summary(summary: Dict, config: Mapping) -> Dict:
    ...
    summary["learning_rate_variant"] = float(config.get("learning_rate", np.nan))
...
def _plot_final_exploitability(
    ...
    set_chart_title(ax, "ESCHER learning-rate ablation: final exploitability")
    ax.legend()
    fig.tight_layout()
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-root", default="outputs/learning_rate_ablation")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    with tqdm(total=total, desc="Learning-rate variants") as progress:
        for seed in seeds:
            for variant in variants:
